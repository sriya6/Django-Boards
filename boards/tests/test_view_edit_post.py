...
class PostUpdateViewTestCase(TestCase):
    '''
    Base testcase to be used in all `PostUpdateView` view tests
    '''
    def setUp(self):
...
class UnauthorizedPostUpdateViewTests(PostUpdateViewTestCase):
    def setUp(self):
        '''
        Create a diff user from the one who posted
        '''
        super().setUp()
        username='jane'
        password='j321'
    ...
    def test_status_code(self):
        '''
        A topic should only be edited by the owner of the post
        Unauthorized users should get 404 error (page not found)
        '''
...
class SuccessfulPostUpdateViewTests(PostUpdateViewTestCase):
    def setUp(self):
        super().setUp()
        self.client.login(username=self.username, password = self.password)
        self.response = self.client.post(self.url, {'message': 'edited_message'})
    ...
    def test_post_changed(self):
        ...
        self.assertEqual(self.post.message, 'edited_message')
